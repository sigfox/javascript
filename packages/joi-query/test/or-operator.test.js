...
const schema = Joi.queryOrOperator({
  firstname: Joi.string(),
  lastname: Joi.string()
});

describe('joi-query: or-operator', () => {
  ...
  describe('with invalid $or child value', () => {
    it('fails', async () => {
      const result = Joi.validate({ $or: { firstname: 42 } }, schema);
      chai
        .expect(result.error.message)
        .to.equal(
          'child "&#x24;or" fails because [child "firstname" fails because ["firstname" must be a string]]'
        );
    });
  });
