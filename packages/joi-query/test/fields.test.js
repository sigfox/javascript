...
const ALLOWED_FIELDS = ['lastname', 'firstname', 'email'];

const schema = Joi.queryFields(ALLOWED_FIELDS);

describe('joi-query: fields', () => {
  ...
  describe('with invalid field', () => {
    it('fails', async () => {
      ...
      chai
        ...
        .to.equal(
          '"value" at position 3 fails because ["3" must be one of [lastname, firstname, email]]'
        );
    });
  });
